...
If a usable Excel file is found in `Data/`, the script automatically loads its
first two numeric columns and uses them as the input (x) and output (y)
variables. If no usable file is found (or pandas/openpyxl aren't installed),
it falls back to a small built-in sample dataset so the script always runs.
...
All plots are stored in the `plots` folder. *(These exploratory plots are
produced separately from `Gradient-Descent-Statistical-Model.py` — they are
not generated by that script.)*

### State-wise Data Analysis

![State-wise Data Analysis](plots/data_by_state.png)

### Population and Growth Rate

![Population](plots/population.png)
...
![Unemployment](plots/unemployment.png)

### Literacy Rate

![Literacy Rate](plots/literacy_rate.png)
